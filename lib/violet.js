...
module.exports = function(appName) {
  ...
  var __getResponse = (req, resp) => {
    ...
    var response = {
      ...
      load: (objNameStr, keyNameStr, keyVal, filter, queryXtra) => {
        ...
        console.log('Cant load object: ' + objNameStr + ' -- no constraints.');
      },
  ...
  var _registerIntentDef = (userSpeech, intentDefArr) => {
    ...
    var iName = _genIntentName();
    _app.intent( iName, intentParams, (requ, resp) => {
  ...
  _app.launch( function( requ, resp ) {
    return _processIntent(requ, resp, null, (response)=>{
      ...
      response.say(['Hey, how can I help?', 'Hey, how are you feeling today?', 'Hi, how can I help?', 'What\'s going on today?']);
    });
  });
  _app.intent('setAlert', {"utterances": ["set alert"]}, (req, resp) => {_setAlert('intent');});
  _app.intent('unsetAlert', {"utterances": ["disable alert", "clear alert"]}, (req, resp) => {_clearAlert('intent');});

  _app.intent('closeSession', {"utterances": ["I am good", "No I am good", "Thanks", "Thank you"]}, () => {}); // by default session ends
