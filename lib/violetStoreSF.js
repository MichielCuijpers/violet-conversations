  ...
  update: (objName, keyName, keyVal, updateData)=>{
    ...
    return org.query({ query: q }).then(function(resp){
      ...
      for(var key in updateData){
        ...
        console.log(_getCompoundNameToStr(key));
...
var nForceWrapper = {
  describe: ()=>{
    org.getDescribe ( "Account", function(err, resp){
        for(var field in (resp.fields)) {
          console.log(resp.fields[field].name);
        }
    });
  }
}
...
module.exports = function(violet) {
  ...
  return {store: sfStore, describer: nForceWrapper};
};
